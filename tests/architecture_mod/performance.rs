...
fn check_performance_recursive(dir: &Path) {
    for entry in fs::read_dir(dir).unwrap() {
        ...
        if path.is_dir() {
            ...
            if path_str.contains("main.rs") || path_str.contains("sets.rs") {
                continue;
            }
            ...
            if code.contains("Query<") && code.contains("&mut Text") {
                ...
                if !is_reactive {
                    panic!(
                        "Performance Violation in {:?}: UI must use Change Detection!",
                        path
                    );
                }
            }
            ...
            if (code.contains(".insert(") || code.contains(".remove::<"))
                && (code.contains("Hungry")
                    || code.contains("InDarkness")
                    || code.contains("Selected"))
            {
                ...
                if !is_reactive {
                    panic!(
                        "Performance Violation in {:?}: Marker management must be reactive!",
                        path
                    );
                }
            }
        ...
        if path.is_dir() {
                ...
                || path_str.contains("atmosphere.rs")
            {
                if code.contains("add_systems(Update,") {
                    panic!(
                        "Architectural Violation in {:?}: Core logic found in 'Update' schedule.",
                        path
                    );
                }
            }
            ...
            if path_str.contains("ui") || path_str.contains("camera.rs") {
                if code.contains("add_systems(FixedUpdate,") {
                    panic!(
                        "Architectural Violation in {:?}: Presentation found in 'FixedUpdate'.",
                        path
                    );
                }
            }
