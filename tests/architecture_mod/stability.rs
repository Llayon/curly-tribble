    ...
    for entry in fs::read_dir(dir).expect("Could not read directory") {
        ...
        if path.is_dir() {
            ...
            let forbidden_patterns = [
                (".unwrap()", "Immediate panic if None/Err. Use 'if let' or 'match'."),
                (".expect(", "Immediate panic with message. Use proper error handling."),
                (".unwrap_err()", "Immediate panic if Ok. Reserved for tests only."),
                ("panic!", "Manual crash detected. All errors must be handled gracefully."),
                ("todo!", "Placeholder found! Logic must be fully implemented for production."),
                ("unimplemented!", "Logic missing! Features must be complete."),
                ("unreachable!", "Dangerous assumption! Handle the 'impossible' case safely."),
            ];
